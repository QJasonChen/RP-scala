...
//    createTri(0,0,"TtB",rand.nextInt(3)+1,draw)
    ...
    def move(tri:Polygon):Polygon={
      ...
      val X=pts(0)/cellSize
      val Y=pts(1)/cellSize
      var dir= tri.accessibleText.toString()
      dir match{
      case "LtR" => triangle(X+fwd,Y,dir,fwd)
      case "RtL" => triangle(X-fwd,Y,dir,fwd)
      case "TtB" => triangle(X,Y+fwd,dir,fwd)
      case "BtT" => triangle(X,Y-fwd,dir,fwd)
      case _ => triangle(X+fwd,Y,dir,fwd)
      }
    }
